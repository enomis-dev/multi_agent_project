...
def process_input(graph, messages, thread: dict, debug=False):
    """
    Processes user input through the LangGraph compiled graph and returns the assistant's response.

    Args:
        graph: The compiled LangGraph instance used for processing messages.
        messages (list): A list of message dictionaries representing the conversation history. 
                         Each dictionary should follow the format:
                         {"role": "user" | "assistant", "content": str}.
        thread (dict): A dictionary representing the thread context, including state or configuration 
                       details required for processing the conversation.
        debug (bool, optional): If True, prints intermediate events during graph execution for debugging purposes. 
                                Defaults to False.

    Returns:
        str: The content of the last message generated by the assistant (agent_A).
    
    Example:
        graph = create_graph(...)
        messages = [{"role": "user", "content": "Hello!"}]
        thread = {"thread_id": 1}
        response = process_input(graph, messages, thread)
        print(response)  # Output: "Hi there! How can I help you?"
    """
    events = graph.stream(
        {
            "messages": messages,
